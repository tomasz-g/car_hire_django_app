...
class CarModel(models.Model):
    ...
    def __str__(self):
        return '{} {}'.format(self.manufacturer_name_and_logo, self.manufacturer_model_name)
...
class Car(models.Model):
    car_make_and_model = models.ForeignKey(CarModel, verbose_name="Make and Model")
    car_body = models.ForeignKey('BodyType')
    car_type = models.ForeignKey('CarType')
    num_of_doors = models.DecimalField(max_digits=1, decimal_places=0, default=4, verbose_name="Doors")
    ...
    def __str__(self):
            return '{} {} {} Doors'.format(self.car_make_and_model, self.car_body, self.num_of_doors)
...
class CarSpecs(models.Model):
    specification = models.CharField(max_length=25, help_text="Enter a car specs (e.g. Audio System, Sunroof, Airbags)")
...
class CarEngineSize(models.Model):
    engine_size = models.DecimalField(max_digits=2, decimal_places=1, default=1.0)
...
class CarInstance(models.Model):
    ...
    rented_to_client = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, verbose_name="Customer Name")
    ...
    def clean(self):
        if self.rented_to_client and self.car_status == 'a':
            raise ValidationError("Status can't be 'Available' on rented car!")
